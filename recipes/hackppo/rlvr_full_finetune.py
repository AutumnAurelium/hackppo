...
class PPORLVRFullFinetuneRecipeSingleDevice(FTRecipeInterface):
    ...
    # Basic configuration
    _device: torch.device  # Training device
    _dtype: torch.dtype   # Training precision (bf16 or fp32)

    # Model output and logging
    _output_dir: str  # Directory for saving checkpoints
    _metric_logger: DiskLogger  # Logger for training metrics
    _log_every_n_steps: int  # How often to log metrics
    _log_peak_memory_stats: bool  # Whether to log GPU memory stats

    # RNG state
    seed: int  # Random seed for reproducibility  
    _rng: torch.Generator  # Random number generator

    # Training progress tracking
    _total_steps: int  # Total optimization steps to run
    _steps_run: int  # Steps completed so far
    _total_epochs: int  # Total epochs to run
    _epochs_run: int  # Epochs completed so far
    global_step: int  # Global step counter (includes PPO epochs)

    # Training configuration
    _resume_from_checkpoint: bool  # Whether to resume from checkpoint
    _gradient_accumulation_steps: int  # Number of gradient accumulation steps
    compile: bool  # Whether to use torch.compile
    _optimizer_in_bwd: bool  # Whether to fuse optimizer step into backward pass

    # Checkpointers
    _policy_checkpointer: Checkpointer
    _value_checkpointer: Checkpointer
    ...
    _optim_ckpt_wrapper: Optional[OptimizerInBackwardWrapper]  # For in-backward optimizer checkpointing
    ...
    # Loss function
    _loss_fn: nn.Module  # PPO loss function

    # PPO hyperparameters
    _kl_coeff: float  # KL penalty coefficient
    _gamma: float  # Discount factor for returns
    _lmbda: float  # GAE lambda parameter
    _whiten_rewards: bool  # Whether to normalize rewards

    # Generation parameters
    _temperature: float  # Sampling temperature
    _top_k: Optional[int]  # Top-k sampling parameter
    _max_generated_tokens: int  # Max tokens to generate

    # Reward configuration  
    _min_response_length: int  # Min required response length
    _penalise_no_eos: bool  # Whether to penalize missing EOS
    _reward_penalty: float  # Penalty value for invalid responses
    _stop_token_ids: torch.Tensor  # Token IDs that stop generation

    # Batch sizes
    batch_size: int  # Total batch size per step
    _forward_batch_size: int  # Batch size for generation
    _ppo_epochs: int  # PPO epochs per batch
    _ppo_batch_size: int  # PPO minibatch size
    _ppo_backward_batch_size: int  # Microbatch size for gradient accumulation

    # Caching
    enable_kv_cache: bool  # Whether to use KV caching
    cache_ctx_manager: Callable[[bool], contextlib.AbstractContextManager]  # Context manager for KV cache

    # Profiling
    _profiler: Union[torch.profiler.profile, DummyProfiler]  # Profiler instance
    profiler_profile_memory: bool  # Whether to profile memory
    profiler_wait_steps: Optional[int]  # Profiler wait steps
    profiler_warmup_steps: Optional[int]  # Profiler warmup steps  
    profiler_active_steps: Optional[int]  # Profiler active steps
